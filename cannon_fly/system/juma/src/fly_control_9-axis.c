...
static void imu_5ms_flag_turn(void);
/*reset sensors*/
imu_status_t imu_sensor_9_axis_reset(void)
{
  ...
  if (LSM6DS3_IO_Init() != imu_status_ok)
  {
	...
	TIM10_IRQFUNC=imu_5ms_flag_turn;
...
imu_status_t imu_sensor_9_axis_set_data_rate(uint32_t* p_data_rate, uint8_t mode)
{
  ...
  /*acc and gyro odr */
  {
    ...
    if (sensor_selection & ACC_ENABLE) {
      ...
      if (imu_sensor_9_axis_config_acc(new_odr, LSM6DS3_XL_FS_8G) != imu_status_ok) {
        return imu_status_fail;
    ...
    if (sensor_selection & GYRO_ENABLE) {
      ...
      if (imu_sensor_9_axis_config_gyro(new_odr, LSM6DS3_G_FS_2000) != imu_status_ok) {
        return imu_status_fail;
      }
    }
...
void imu_sensor_9_axis_read_data_from_fifo(void)
{
  ...
  //sensor_data.gyro[2] = ((float)pData[2] / 32768 * 500 );
	sensor_data.gyro[0] = (float)(pData[0]* GYRO_SCALE * M_PI_F /180.f);
	sensor_data.gyro[1] = (float)(pData[1]* GYRO_SCALE * M_PI_F /180.f);
	sensor_data.gyro[2] = (float)(pData[2]* GYRO_SCALE * M_PI_F /180.f);
	
  //sensor_data.acc[0] = ((float)pData[3] / 32768 * 2)*1000;
  //sensor_data.acc[1] = ((float)pData[4] / 32768 * 2)*1000;
  //sensor_data.acc[2] = ((float)pData[5] / 32768 * 2)*1000;
	sensor_data.acc[0] = (float)(pData[3]*ACC_SCALE * CONSTANTS_ONE_G);
	sensor_data.acc[1] = (float)(pData[4]*ACC_SCALE * CONSTANTS_ONE_G);
	sensor_data.acc[2] = (float)((-pData[5])*ACC_SCALE * CONSTANTS_ONE_G);
...
void imu_5ms_flag_turn(void)
{
	if(flag_5ms == Flag_SET)
		flag_5ms = Flag_RESET;
}
/*****************End Of File*******************/