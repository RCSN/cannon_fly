...
void imu_sensor_read_data_from_fifo(void* arg)
{
    ...
    while(number > 0) {
        ...
        if(flag == TYPE_GYRO_DATA) {
            ...
            //sensor_data.gyro[2] = (float)((pData[2] * sensor_data_sensitivity.gyro_sensitivity)/1000);
						sensor_data.gyro[0] = (float)pData[0];
						sensor_data.gyro[1] = (float)pData[1];
						sensor_data.gyro[2] = (float)pData[2];
            flag = TYPE_ACC_DATA;
            ...
            //sensor_data.acc[2] = (float)(pData[2] * sensor_data_sensitivity.acc_sensitivity);
						sensor_data.acc[0] = (float)pData[0];
						sensor_data.acc[1] = (float)pData[1];
						sensor_data.acc[2] = (float)pData[2];
    ...
    //run_after_delay(imu_sensor_read_data_from_fifo, NULL, sensor_data_param.delay_time);
		ANO_DT_Send_Senser(sensor_data.acc[0],sensor_data.acc[1],sensor_data.acc[2],sensor_data.gyro[0],sensor_data.gyro[1],sensor_data.gyro[2],sensor_data.mag[0],sensor_data.mag[1],sensor_data.mag[2],0);
		run_after_delay(imu_sensor_read_data_from_fifo, NULL, 5);
}
