...
uint8_t  RX_ADDRESS[RX_ADR_WIDTH]= {0x00,0x00,0x00,0x00,0xA1};	//接收地址				
...
uint8_t NRF_Read_Reg(uint8_t reg)
{
    ...
    HAL_SPI_Receive(&hnucleo_Spi2,  &reg_val, 1, 100);	  /* 读取该寄存器返回数据 */
    SPI2_CSN_H();	
...
uint8_t NRF_Write_Buf(uint8_t reg, uint8_t *pBuf, uint8_t uchars)
{
    ...
    uint8_t status;
    SPI2_CSN_L();				        /* 选通器件 */
    status = HAL_SPI_Transmit(&hnucleo_Spi2,&reg, 1, 100);	/* 写寄存器地址 */
//    for(i=0; i<uchars; i++)
//    {
//        SPI2_RW(pBuf[i]);		/* 写数据 */
//    }
...
void SetRX_Mode(void)
{
    SPI2_CE_L();
  	...
  	NRF_Write_Reg(NRF_WRITE_REG+RF_CH,0);	     //设置RF通信频率		  
